<x-app-layout>
    <x-slot name="header"> 
    <div class="font-semibold text-xl text-gray-800 leading-tight">{{__('Edit presentacion')}}</div >
    ...
    <div class="container mx-auto transform translate-y-6 w-full">
        ...
        <div class="bg-white p-4 border-solid rounded">
                ...
                <tr>
                    <form action="{{ route('presentacion_i.update', $imagen->id_imagen) }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
                    @csrf
                    @method('PUT')
                    <div class="flex flex-wrap -mx-4 mb-2">    
                        ...
                        <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2">URL Image</label> 
                            <input value="{{$imagen->url_imagen}}" name="url_imagen" class="appearance-none block w-full bg-gray-200 text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500" type="text" name="input2"><br>
                        </div>
